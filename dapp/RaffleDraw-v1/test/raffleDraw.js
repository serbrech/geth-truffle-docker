...
contract('RaffleDraw', function(accounts) {
  ...
  it("will check Bob(admin) can add users", function() {
      ...
      contract.users(2).then ( user => { assert.equal(user, "Carl", "Bob should be able to add users") }); 
  });
  ...
  //     return contract.TestRandom.call().then( random => { console.log("A random number is [0 to 2] :" + random) }); 
  // });

  // it("will draw a winner and check only 2 users left", function() {
  //     contract.DrawUser();
  ...
  //     return contract.NbUsers.call().then( nbUsers => { assert.equal(nbUsers, 2, "Only 2 users should remains"); }); 

  //     //HOW TO DO AN assert.contains(...)?
  //     //return contract.users.call(3).catch ( error => { assert.contains(error.message, "VM Exception while executing eth_call: invalid opcode") }); 
  //     // , "The winning user should be deleted"
  //     //return contract.users.call(3).catch ( error => { console.log(error.message) }); 
   // });

// NOT WORKING, not sure why the return value is not the winner??
  // it("will draw a winner, then re-draw a new winner while adding the previous one in pool, and check only 1 user left", function() {
  //     contract.users.call(0).then( user => { console.log("- User[0]:"+ user) });
  //     contract.users.call(1).then( user => { console.log("- User[1]:"+ user) });
  //     contract.users.call(2).then( user => { console.log("- User[2]:"+ user) });
  //     contract.NbUsers.call().then( nbUsers => { console.log("NbUsers remaining:"+ nbUsers) });

  //     contract.ReDrawUser("Elon");
  //     contract.ReDrawUser.call("Elon").then( newwinner => { 
  ...
  //     contract.users.call(1).then( user => { console.log("- User[1]:"+ user) });
  //     //contract.users.call(2).then( user => { console.log("- User[2]:"+ user) });
  //     return contract.NbUsers.call().then( nbUsers => { console.log("NbUsers remaining:"+ nbUsers) });
  // });


// SAME NOT WORKING, not sure why the return value is not the winner??
  // it("will draw a winner, then re-draw a new winner while adding the previous one in pool, and check only 1 user left", function() {
  //     return contract.DrawUser().then( tx => {
  //       return contract.DrawUser.call().then( user => { 
  //           console.log("=> First winner: " + user);
  //           return contract.ReDrawUser(user).then( tx => { 
  //               contract.ReDrawUser.call(user).then( user2 => { console.log("=> new winner: " + user2) });  
  //               return contract.NbUsers.call().then( nbUsers => { console.log("NbUsers remaining:"+ nbUsers) });
